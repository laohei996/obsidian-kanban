...
当前 75 个用例直接导入实际实现，不复制业务算法或词典：40 个产品行为用例，以及 35 个构建元数据用例。
...
- **本地化运行时**：语言别名/区域回退、未知语言、真实空 Hindi 词典的英文 fallback、同模块语言切换、特殊字符与数字插值。与静态词典检查互补。
...
测试只模拟所需的 `app` 配置/插件启用状态与 `window.localStorage.getItem`；不引入完整 Obsidian 或 DOM 替身。
